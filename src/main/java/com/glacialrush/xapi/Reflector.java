...
{
	private GList<String> log;
	
	public Reflector()
	{
		log = new GList<String>();
	}
	...
	{
		GList<MetaReference> mr = getMetaReferences(clazz, null);
		MetaData md = new MetaData(clazz.getName());
		md.setReferences(mr.toArray(new MetaReference[mr.size()]));
		return md;
	}
	
	private GList<MetaReference> getMetaReferences(Class<?> clazz, String base)
	{
		GList<MetaReference> references = new GList<MetaReference>();
		GList<Class<?>> used = new GList<Class<?>>();
		...
		for(Field i : clazz.getDeclaredFields())
		{
			if(isValidField(i))
			{
				...
				{
					GList<MetaReference> iReferences = getMetaReferences(i.getType(), new MetaTarget(new MetaTarget(base), i.getName()).toString());
